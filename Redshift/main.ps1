# ==============================================================================
# AWS REDSHIFT SERVERLESS (PASSWORD NUMERIC VALIDATION FIX)
# ==============================================================================
Write-Host ">>> Initializing Redshift Serverless Deployment Process..." -ForegroundColor Cyan

# -----------------------------------------------------------------------------
# STEP 0: PURGE STALE TERRAFORM STATE & CLEAN UP EXISTING INFRASTRUCTURE
# -----------------------------------------------------------------------------
Write-Host "`n[0/4] Checking and cleaning local state/infrastructure..." -ForegroundColor Yellow

$OldErrorAction = $ErrorActionPreference
$ErrorActionPreference = "SilentlyContinue"

if (Test-Path "$PWD/main.tf") {
    Write-Host "Existing Terraform configuration detected. Initiating teardown..." -ForegroundColor Red
    terraform destroy -auto-approve *>$null
    Write-Host "Cleanup complete." -ForegroundColor Green
} else {
    Write-Host "No existing main.tf configuration found. Skipping teardown." -ForegroundColor Green
}
...
# -----------------------------------------------------------------------------
Write-Host "`n[1/4] Generating Lab-Compliant Terraform configuration..." -ForegroundColor Yellow
...
# -----------------------------------------------------------------------------
Write-Host "`n[2/4] Running Terraform Init & Apply..." -ForegroundColor Yellow
...
Write-Host "`n>>> Redshift Serverless infrastructure deployed successfully!" -ForegroundColor Green